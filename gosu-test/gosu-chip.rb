...
# Layering of sprites  
module ZOrder  
  Background, Stars, Player, UI, Cursor = *0..4  
    
    
end  
class Cursor  
  attr_reader :img, :visible, :imgObj, :pos_x, :pos_y  
  def initialize(window,img,visible)  
    ...
    @forced=false  
    @window = window  
    @pos_x = @window.mouse_x  
    @pos_y = @window.mouse_y  
  end  
  def toggleVisible  
    if not @forced  
      @visible=!visible  
...
class Player  
  ...
  def initialize(window, shape, cursor)  
  ...
  def update  
    angle_degrees = Gosu::angle(@shape.body.p.x, @shape.body.p.y, @cursor.pos_x , @cursor.pos_y )  
    #puts angle_degrees  
    @shape.body.a = angle_degrees.gosu_to_radians  
    #   #TODO: apply a smooth rotation (e.g. pontential) that dimish torque when reaching goal  
