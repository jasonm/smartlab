		...
		if (address.equals("/tuio/2Dobj")) {
			...
			if (command.equals("set")) {
				...
				if (objectList.get(s_id) == null) {
					...
					TuioObject addObject = new TuioObject(currentTime,s_id,c_id,xpos,ypos,angle);
					...
					if ((updateObject.xpos!=xpos) || (updateObject.ypos!=ypos) || (updateObject.angle!=angle) || (updateObject.x_speed!=xspeed) || (updateObject.y_speed!=yspeed) || (updateObject.rotation_speed!=rspeed) || (updateObject.motion_accel!=maccel) || (updateObject.rotation_accel!=raccel)) {
						...
						TuioObject tobj = new TuioObject(currentTime,s_id,c_id,xpos,ypos,angle);
						tobj.update(currentTime,xpos,ypos,angle,xspeed,yspeed,rspeed,maccel,raccel);
						frameObjects.addElement(tobj);
				...
				//System.out.println("set obj " +s_id+" "+c_id+" "+xpos+" "+ypos+" "+angle+" "+xspeed+" "+yspeed+" "+rspeed+" "+maccel+" "+raccel);
			...
			if (command.equals("set")) {
				...
				if (cursorList.get(s_id) == null) {
					...
					if (cursorList.size()<=maxCursorID) {
					...
					TuioCursor addCursor = new TuioCursor(currentTime,s_id,c_id,xpos,ypos);
					...
					if ((updateCursor.xpos!=xpos) || (updateCursor.ypos!=ypos) || (updateCursor.x_speed!=xspeed) || (updateCursor.y_speed!=yspeed) || (updateCursor.motion_accel!=maccel)) {
						...
						TuioCursor tcur = new TuioCursor(currentTime,s_id,updateCursor.getCursorID(),xpos,ypos);
						tcur.update(currentTime,xpos,ypos,xspeed,yspeed,maccel);
						frameCursors.addElement(tcur);
				...
				for (int i=0;i<aliveCursorList.size();i++) {
					...
					} else if (removeCursor.getCursorID()<maxCursorID) freeCursorList.addElement(removeCursor);
